  ...
  function initMap() {
    ...
    const sendGeoCord = document.createElement("submit");
    sendGeoCord.textContent = "Select this zone";
    sendGeoCord.classList.add("sendZone");
    map.controls[google.maps.ControlPosition.BOTTOM_CENTER].push(sendGeoCord);
    var imported = document.createElement('script');
    imported.src = 'jStorage/jstorage.min.js';
...
sendGeoCord.addEventListener("click", () => {
   $.jStorage.flush();
  for(var key in getOutCord){
 
    insertvalue(key,getOutCord[key]);
  }
...
function insertvalue(key,val){
  ...
  function outputGeometry() {
   ...
   getOutCord = {
      cordLatitute: geo.lat,
      cordLongitude: geo.lng,
      cordRadianti: geo.radinanti_km
  };
